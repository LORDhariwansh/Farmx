		...
		rectangle_62.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if(TextUtils.isEmpty(usernamer)){
					Toast.makeText(login_act.this, "ENTER USERNAME", Toast.LENGTH_SHORT).show();
					return;
				}
				...
				Auth.signInWithEmailAndPassword(usernamer, pass)
						.addOnCompleteListener( new OnCompleteListener<AuthResult>() {
							@Override
							public void onComplete(@NonNull Task<AuthResult> task) {
								if (task.isSuccessful()) {
									Toast.makeText(login_act.this, "LOGIN DONE", Toast.LENGTH_SHORT).show();
									Intent intent=new Intent(getApplicationContext(), MainActivity.class);
									startActivity(intent);
								} else {
									Toast.makeText(login_act.this, "FAILED", Toast.LENGTH_SHORT).show();
